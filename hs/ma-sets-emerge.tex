...
Let's also formulate what we mean by converging to the "closest fixed point" more precisely. Let's suppose there is a sequence of fixed points with length at least 1, where $x_0 < c_1 < c_2 < ...$. We wish to prove that the sequence $\{x_n\}$ converges to $c_1$.
...
\end{lemma}
\begin{proof}
We prove this by induction. Our base case is easy: we're simply given that $x_0 < c_1$.
...
\end{lemma}
\begin{proof}
We'll prove that for all $k \geq 0$, $x_0 \leq x_k < x_{k+1}$ (i.e. both $x_0 \leq x_k$, and $x_k < x_{k+1}$). 

For our base case, clearly $x_0 \leq x_0$. We are given that $f(x_0) > x_0$, and by definition of our sequence, $x_1 = f(x_0)$.

For our inductive step, $x_0 \leq x_k < x_{k+1}$, and prove that $x_0 \leq x_{k+1} < x_{k+2}$. Since $x_0 \leq x_k < x_{k+1}$, we clearly have $x_0 \leq x_{k+1}$. Per Lemma \ref{l:fp-iter-bound}, we have $x_{k+1} < c_1$. Therefore, we have that $x_{k+1} \in \left[x_0, c_1\right)$, so by Lemma \ref{l:fp-iter-region}, $f(x_{k+1}) > x_{k+1}$. We simply then have $x_{k+2} = f(x_{k+1})$, and therefore $x_{k+1} < x_{k+2}$. 
...
We finally show that, therefore, $\{x_n\}$ must converge to $c_1$. As for the fixed points greater than $c_1$, clearly none of them can be a least upper bound by definition, so converge to them is out of the question.
...
This all leaves us with only one possibility: $\{x_n\}$ converges to $c_1$, its lowest fixed point to the right.
