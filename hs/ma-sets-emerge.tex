...
Suppose $b < c_0$. We know from Part (c) that $b$ has to be a fixed point of $f$. We know $c_0$ is a lower bound for the fixed points of $f$ to the right of $x_0$,\footnote{Specifically the greatest lower bound, of course.} and we know that $x_0$ is a not a fixed point, since we're given that $f(x_0) > x_0$.

Therefore, if $b < c_0$, then we also know that $b < x_0$. 

Then to show that $\lim_{n \rightarrow \infty} x_n \neq b$, consider $\epsilon = (x_0 - b)/2$. From Lemma \ref{l:fp-iter-increasing-lower-bound}, we know that for \emph{every} $n$, we have $x_n > x_0$ -- and as a consequence:
...
Therefore, we've found an $\epsilon$ that we cannot satisfy, no matter how big $n$ is. Therefore, the limit cannot be $b$ if $b < c_0$. 
...
Consider $\epsilon = b - c_0$. Note that $|x_n - b| = b - x_n$ (since $b >= x_n$ since $b$ is an upper bound of the sequence as mentioned above). Therefore, we'd have:
