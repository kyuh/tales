...
Let's also formulate what we mean by converging to the "closest fixed point" more precisely. Suppose we have a set of fixed points $\{c : f(c) = c\}$ which is non-empty, and therefore has a greatest lower bound $c_1$. 

From Problem \ref{continuous-fn-dense-set}, we know that, since $f$ is continuous, $c_1$ is in this set, and hence is a fixed point by definition.\footnote{This is important -- if we didn't establish this fact, then it's not clear that $c_1$ is a fixed point at all, and hence we wouldn't be proving our claim!} We wish to prove that the sequence $\{x_n\}$ converges to $c_1$.
