...
% TODO: Probably should cover/work with density at some point before this
\begin{problem}
\label{set-inf-inclusion-density}
Let $A$ be a nonempty set that's bounded below, and let $a = \inf A$.

Prove that at least one of the following will be true:
\begin{enumerate}[(1)]
\item $a \in A$
...
\vspace{10mm}
% TODO: With the added question, this transition isn't smooth
...
\item Suppose we instead had $m$ starting amoebas. What is the probability of dying before the $n$-th generation now, in terms of our "single amoeba generation death" probability $P_d(n)$?
...
% We'll show that we get a contradiction if we assume both (1) and (2) are false.

Let $a = \inf A$. By definition, $x \geq a$ for every $x \in A$.

Suppose (2) is false -- in other words, suppose there is some $\epsilon > 0$ such that $x \notin A$ for \textit{every} $x$ in the interval $a < x < a + \epsilon$. 
...
\subsubsection{Case 1: There exist elements $x \geq a + \epsilon$}

Suppose (1) is false. Then $x$ cannot equal $a$, so every $x \in A$ satisfies $x \geq a + \epsilon$. 
...
\subsubsection{Case 2: There exist \textit{no} elements $x \geq a + \epsilon$}
...
&= f(\lim_{n \rightarrow \infty} x_{n-1}) && \text{(From Problem \ref{pr:lim-cont-func-seq}, since $f$ is continuous)} \\
&= f(c) && \text{(Problem \ref{pr:lim-offset-seq})} \\
\end{align*}
...
Then there is no way that $\lim_{x \rightarrow a} g(x) = g(a)$. To show this, take (for example) $\epsilon = b/2$. Then for any $\delta > 0$, I would have a point $x$ where $a < x < a + \delta$, but $|g(x) - g(a)| = |0 - b| = b > \epsilon$.\footnote{Note: This is a much more general statement than saying that $\lim_{x \rightarrow a} g(x) \neq g(a)$, because we don't know that any limit exists at all. But we can say that the the statement $\lim_{x \rightarrow a} g(x) = g(a)$ is false, and that is the key.}
% TODO: Fill in the above limit statement in the footnote -- "the statement <limit>, i.e. <spell it out>"

Then by definition, $g$ is not continuous at $a$. Contradiction. $\square$
...
We'll do a proof by contradiction. Suppose this weren't true, i.e. there existed at least one point $x_a$ in the region $\left[x_0, c_0\right)$ such that $f(x_a) < x_a$, i.e. $g(x_a) < 0$. We're given (in the problem) that $f(x_0) > x_0$, i.e. $g(x_0) > 0$. Therefore, by the intermediate value theorem, there exists a point $x_b$ in $(x_0, x_a)$ such that $g(x_b) = 0$, i.e. $f(x_b) = x_b$. 
...
From Lemmas \ref{l:fp-iter-bound} and \ref{l:fp-iter-increasing-lower-bound}, we see that $\{x_n\}$ is an increasing and bounded sequence. Therefore, by Theorem \ref{th:monotone-seq}, $\{x_n\}$ converges to its least upper bound, which we'll call $b$. 
...
% TODO: Figure would be helpful here (and in many other places around here)
Then to show that $\lim_{n \rightarrow \infty} x_n \neq b$, consider $\epsilon = x_0 - b$. From Lemma \ref{l:fp-iter-increasing-lower-bound}, we know that for \emph{every} $n$, we have $x_n \geq x_0$, and therefore:

\begin{align*}
|x_n - b| &= x_n - b && \text{(since $x_n > b$, because $x_n \geq x_0$ and $x_0 > b$)} \\
&\geq x_0 - b \\
&= \epsilon
\end{align*}
