...
Let's clarify the second case a bit more. If our current amoeba has lived to reproduce, then the child amoebas have $(n-1)$ generations to go in reaching our original $n$-generation goal. The only way for the family to die out before $n$ generations is if \emph{both} of our children amoebas produce families that die out before $(n-1)$ generations, respective to them. By our definition, each such child amoeba has probability $P_d(n-1)$ of that occurring.

This looks promising recurrence-relation wise! Let's solidify our relation a bit more formally.
